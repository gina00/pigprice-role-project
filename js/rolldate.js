...
(function (global, factory) {
  ...
  Rolldate.prototype = {
      ...
      baseData: function baseData() {
          return {
              ...
              opts: { //插件默认配置
                  ...
                  customerData:null//自定义数据不用生成时分秒
              }
          };
      },
      ...
      createUI: function createUI() {
          ...
          for (var i = 0; i < len; i++) {
              ...
              if(config.customerData){
                for (var index in  config.customerData) {
                    itemClass = data == '' ? 'active' : '';

                    ul += '<li class="wheel-item ' + itemClass + '" data-index="' + domMndex +
                    +'" value="'+config.customerData[index].code +
                    '">' + config.customerData[index].title + '</li>';
                    domMndex++;
                }
              }
              if (f == 'YYYY') {
                  ...
                  for (var o = 0; o <= 59; o++) {
                      ...
                      ul += '<li class="wheel-item ' + itemClass + '" data-index="' + domMndex + '">' + (o < 10 ? '0' + o : o) + lang.sec + 'sss</li>';
                      domMndex++;
                  }
              }
              ul += '</ul></div>';
      ...
      event: function event() {
          ...
          _this.tap(confirm, function () {
              ...
              for (var f in _this.scroll) {
                  if(f=='cus'){
                     value=_this.getSelectedText(_this.scroll[f]);
                     break;
                  }
      ...
      getSelected: function getSelected(scroll) {
           var el=this.$('#' + scroll.wrapper.id + ' li', 1);
          var index=scroll.getSelectedIndex();
         return el[index].innerText.replace(/\D/g, '');
      },
      getSelectedText: function getSelected(scroll) {
                   var el=this.$('#' + scroll.wrapper.id + ' li', 1);
                  var index=scroll.getSelectedIndex();
                 return el[index].innerText;
              },
  };
